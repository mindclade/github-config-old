...
projects, and the audit-only supply-chain inventory, change `ENVIRONMENT_PROJECT_IDS` to
`env:ENVIRONMENT_PROJECT_IDS`. From the exact clean applied infrastructure commit, generate the
mode-0600 handoff outside both repositories, then run the full exporter against that exact revision:

```sh
python3 ../infrastructure-live/scripts/export-applied-control-plane-handoff.py \
...
python3 scripts/export-ci-variables.py \
  --bootstrap ../bootstrap \
  --infrastructure-handoff /protected/evidence/infrastructure-control-plane-handoff.json \
  --expected-infrastructure-commit "$MERGED_INFRASTRUCTURE_SHA" \
  > /tmp/github-config-ci-variables.json
```

The handoff supplies `SA_GITOPS_RENDER`, `SA_GITOPS_VERIFIER`, the retained v3 signer, and exact
attestor inventory. Same-named environment variables are never accepted. Contract version 1.1.0
also proves Binary Authorization is audit-only and ARC activation is disabled; publishing this
inventory does not authorize a release or cluster synchronization. Full mode remains fail-closed
on every unresolved non-Buildkite input.
