...
def compile_payload(bootstrap: Path) -> dict[str, dict[str, Any]]:
    config = resolve_environment(
        run_json(["yq", "-o=json", ".", str(ROOT / "catalog/ci-variables.yaml")])
    )
    ...
    if not isinstance(buildkite, dict) or buildkite.get("enabled") is not True:
        raise ValueError(
            "bootstrap platform_contract has Buildkite federation disabled"
        )
    ...
    config["bootstrap"].update(
        {
            "BOOTSTRAP_FOLDER_ID": need(
                platform, "bootstrap_folder_id", "platform_contract"
            ),
    ...
    config["infrastructure-live"].update(
        {
            ...
            "BUILDKITE_WIF_POOL_NAME": buildkite_pool,
...
def main() -> int:
    args = parse_args()
    try:
        payload = compile_payload(args.bootstrap.resolve())
